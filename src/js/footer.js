document.addEventListener('DOMContentLoaded', () => {
    ...
    image.width = 340;
    image.height = 20;
    image.src = "https://www.pichincha.com/content/published/api/v1.1/assets/CONT09075459D63942ABBDD4B455D51C7B05/native?channelToken=712a6518832146c488cdf196228d8c00";
  ...
  function initializeFooter() {
    // Coloca aquí el código JavaScript para inicializar el footer
    document.addEventListener('DOMContentLoaded', function() {
      // Crear plantillas y clonar contenido
      const socialIconsTemplate = document.createElement('template');
      socialIconsTemplate.innerHTML = `
      ...
      const legalInfoTemplate = document.createElement('template');
      legalInfoTemplate.innerHTML = `
        <div class="legal-info">
          <a href="#">Términos y condiciones</a> | <a href="#">Aviso legal</a>
        </div>
      `;
  
      const socialIconsClone = socialIconsTemplate.content.cloneNode(true);
      document.querySelector('.mi-footer').appendChild(socialIconsClone);
  
      const legalInfoClone = legalInfoTemplate.content.cloneNode(true);
      document.querySelector('.mi-footer').appendChild(legalInfoClone);
    });
  }
  
  // Llamar a la función para inicializar el footer cuando se cargue la página
  initializeFooter();
  ...
  document.addEventListener('DOMContentLoaded', () => {
    // Crear la estructura del contenido principal
    const mainContentTemplate = document.createElement('template');
    mainContentTemplate.innerHTML = `
        <div class="main-content">
            <div class="info">
                <h1>Banco Pichincha</h1>
                <p>Bienvenido a su banco en línea</p>
                <img src="https://media-public.canva.com/5_kDM/MAEa5h5_kDM/1/t.png" alt="Logo Banco Pichincha">
            </div>
            <div class="login">
                <form action="#" method="post">
                    ...
                    <div class="forgot-password">¿Olvidó su contraseña?</div>
                </form>
            </div>
        </div>
    `;
  
    // Insertar el contenido del template en el main
    const main = document.getElementById('main-template');
    const content = mainContentTemplate.content.cloneNode(true);
